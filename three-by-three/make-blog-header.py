# Make a PNG file containing all the 3x3 weave mazes, in a 900x200 px image.

import cairo

RANDOM_ORDER = True # Set to True to position the mazes at random
...
MARGIN = 120
H_GAP_BETWEEN_CELLS = 77
V_GAP_BETWEEN_CELLS = 40
...
logical_dimen = MARGIN + 16 * (C+B+C+B+C) + 15 * V_GAP_BETWEEN_CELLS + MARGIN
...
def draw(path):
  ...
  c.save()
  scale_factor = 0.86 * float(PAGE_SIZE_PTS) / logical_dimen
...
if RANDOM_ORDER:
  import random
  shuffled_cells = [(x,y) for x in range(8) for y in range(32)]
...
class cell(object):
  ...
  def __enter__(self):
    c.save()
    c.translate(
      MARGIN + (C+B+C+B+C+H_GAP_BETWEEN_CELLS) * self.cell_col - B,
      MARGIN + (C+B+C+B+C+V_GAP_BETWEEN_CELLS) * self.cell_row - B,
    )
...
def main():
  global c
  surface = cairo.ImageSurface(cairo.FORMAT_RGB24, 900, 200)
  c = cairo.Context(surface)

  # White background
  c.rectangle(0,0, 900, 200)
  ...
  # Save the PNG file
  surface.write_to_png("blog-header.png")
  surface.finish()
